...
**심사관의 공격**: 
"이 구성요소(예: BERT, GAN)는 이 기술 분야에서 흔한 관용기술이므로 진보성 없다."

**반박의 초점**:
- **이 구성요소 자체가 이 분야의 관용기술이 아님을 논증**
- 구성요소의 **출처, 표준성, 응용 방식**에 집중
- "D1에서 본 발명으로의 변경 과정"은 언급 금지 (SIMPLE_DESIGN 영역)

## SIMPLE_DESIGN과의 명확한 구분

- COMMON_TECHNIQUE (본 카테고리): "이 구성요소가 왜 관용기술이 아닌가"
- SIMPLE_DESIGN: "D1에서 이 방식으로의 변경이 왜 자명하지 않은가"

**본 카테고리에서 "변경", "변화", "D1 대비 개선" 같은 표현 사용 금지**.
...
## rebuttal 작성 가이드 (COMMON_TECHNIQUE 전용 논거)

다음 중 **최소 하나 이상의 논거**를 구체적으로 서술:

### 논거 1: 다른 분야에서 도입된 구성요소
- 이 구성요소가 원래 어느 분야의 기술인지 밝힘
- 본 발명 분야에서는 새로운 도입임을 강조
- 예: "BERT는 자연어 처리 분야의 기술로, 얼굴 감정 인식 분야에서 도입 사례 없음"

### 논거 2: 이 분야의 표준 아님
- 본 발명 분야의 유사 특허/논문에서 이 구성요소가 사용되지 않음
- 표준 접근이 아니라는 근거
- 예: "얼굴 감정 인식 분야에서 FC 레이어가 표준이며, 어텐션 기반 접근은 비표준"

### 논거 3: 특유 응용 방식
- 원래 용도와 다른 방식으로 사용됨
- 이 방식이 자연스럽게 도출되지 않음
- 예: "BERT는 원래 텍스트 시퀀스용이나, 본 발명은 얼굴 특징 벡터에 적용"

### 논거 4: 다른 구성요소와의 상승효과
- 본 발명의 다른 구성요소와 결합
- 개별 관용기술로만 봐서는 도출 어려운 시너지
- 예: "CNN(A)와 BERT(B)의 결합으로 시각-시맥락 통합 분석이라는 새 능력 창출"

**논거 4가 가장 강력**: 개별 관용기술 여부보다 결합 효과 강조.

## 사용자 입력 활용

- **prior_art_reference**: 사용자가 언급한 유사 선행기술
  → 이 분야 표준 논거(논거 2)에 반영
  → 예: "논문 X도 표준 접근을 사용했으나 본 발명은..."
...
- target_name은 해당 구성요소의 실제 이름 그대로
- 발명 설명에 없는 효과나 특징을 창작 금지
- **"D1에서 변경", "D1 대비 개선", "정확도 X% 향상" 같은 표현 사용 금지**
  (이는 SIMPLE_DESIGN 영역이며 본 카테고리는 구성요소 자체를 논증)
...
```
{
  ...
  "rebuttal": "반박 논리 (150~250자, 구성요소 출처/표준성/응용방식에 초점)"
}
```

## 예시 (SIMPLE_DESIGN과 확연히 다른 톤)

### 예시 1: 다른 분야 도입 + 상승효과 (G06N)
...
[D1]
청구항: CNN + FC 레이어 기반 얼굴 감정 인식

출력:
{
  "target_label": "B",
  "target_name": "BERT 임베딩 모듈",
  "rebuttal": "BERT 임베딩 모듈은 원래 자연어 처리(NLP) 분야의 어텐션 아키텍처로, 얼굴 감정 인식 분야에서는 도입 사례가 없음. 이 분야의 유사 특허들은 FC 레이어 기반이 표준이며 어텐션 접근은 비표준적. 본 발명은 BERT를 텍스트가 아닌 얼굴 특징 벡터에 적용하여, CNN(A)의 시각 특징과 결합해 시맥락 통합 분석이라는 새 능력 창출."
}

### 예시 2: 표준 아님 + 특유 응용 (G06T)

[사용자 발명]
명칭: U-Net 개선 저조도 노이즈 제거
구성요소:
A. 개선된 U-Net 인코더
B. Attention Gate 모듈
C. 실시간 프레임 출력부
...
[D1]
청구항: 기존 U-Net 노이즈 제거 (Attention Gate 미사용)

출력:
{
  "target_label": "B",
  "target_name": "Attention Gate 모듈",
  "rebuttal": "Attention Gate는 원래 의료 영상 세그멘테이션에서 관심 영역 강조용으로 사용되던 기술로, 저조도 노이즈 제거 분야에서는 도입 사례가 없어 관용기술이라 볼 수 없음. 본 발명은 이를 저조도 이미지의 노이즈 영역에 적용하여, U-Net(A)의 검출과 결합해 저조도 특유 텍스처 보존이라는 특유 효과 도출."
}
...
**심사관의 공격**: 
"D1에서 이 부분을 이 방식으로 바꾼 것은 통상의 설계 변경이므로 진보성 없다."

**반박의 초점**:
- **D1에서 본 발명으로의 변경 과정이 자명하지 않음을 논증**
- **변경 동기, 변경 과정의 장벽, 변경 결과의 효과**에 집중
- "이 구성요소가 관용기술이냐 아니냐"는 언급 금지 (COMMON_TECHNIQUE 영역)

## COMMON_TECHNIQUE와의 명확한 구분

- COMMON_TECHNIQUE: "이 구성요소가 왜 관용기술이 아닌가"
- SIMPLE_DESIGN (본 카테고리): "D1에서 이 방식으로의 변경이 왜 자명하지 않은가"

**본 카테고리에서 "관용기술 아님", "이 분야 표준 아님" 같은 표현 사용 금지**.
...
## non_obviousness 작성 가이드 (SIMPLE_DESIGN 전용 논거)

다음 중 **최소 하나 이상의 논거**를 구체적으로 서술:

### 논거 1: 변경 동기 부재
- D1에서 이 방향으로 변경할 이유가 없었음
- 통상의 기술자가 이 변경을 시도할 근거 부재
- 예: "D1은 정확도를 위해 심층 CNN을 사용하는 방향이며, 얕은 어텐션으로 변경할 동기 없음"

### 논거 2: 변경 과정의 기술적 장벽
- 변경 시 발생하는 기술적 문제
- 통상의 방법으로 시도하면 실패했을 것
- 예: "FC 레이어를 어텐션으로 변경 시 계산 비용 폭증하는 장벽이 있었으며, 본 발명은 이를 특유 최적화로 극복"

### 논거 3: 변경 결과의 예상 밖 효과 (가장 강력)
- D1으로부터 예측 불가능한 정량적/정성적 개선
- 수치 근거 있으면 강력
- 예: "이 변경만으로 정확도 7% 향상은 통상의 파라미터 조정으로 도출 불가한 예상 밖 효과"

### 논거 4: 변경된 구성요소의 결합 시너지
- 변경된 구성요소가 D1의 다른 구성과 결합하여 특유 효과 생성
- 개별 변경으로는 도출 안 되는 시너지
- 예: "어텐션(B)으로의 변경과 기존 CNN(A)의 결합으로 시간축 특징 포착이라는 새 능력"

**논거 3(예상 밖 효과)이 가장 강력**. 수치 데이터 있으면 반드시 활용.

## 수치 데이터 활용 (있는 경우)

사용자 발명이 D1 대비 정량적 개선이 있으면 논거 3에 반드시 반영:
...
- **prior_art_reference**: 사용자가 언급한 유사 선행기술
  → 논거 2(기술적 장벽) 근거로 반영
  → 예: "논문 X도 이 변경을 시도했으나 장벽 극복 실패"

- **differentiation_notes**: 사용자가 명시한 차이점
  → 논거 1(변경 동기 부재), 논거 3(예상 밖 효과)에 반영
...
- 발명 설명에 없는 효과나 특징을 창작 금지
- **"이 구성요소는 관용기술 아님", "이 분야 표준 아님", "다른 분야에서 도입" 같은 표현 사용 금지**
  (이는 COMMON_TECHNIQUE 영역이며 본 카테고리는 변경 과정을 논증)
- 150자 미만은 근거 부족, 250자 초과는 장황함
...
```
{
  ...
  "non_obviousness": "비자명성 논리 (150~250자, D1→본발명 변경 과정에 초점)"
}
```

## 예시 (COMMON_TECHNIQUE와 확연히 다른 톤)

### 예시 1: 변경 동기 부재 + 예상 밖 효과 (G06N)

[사용자 발명]
명칭: 딥러닝 기반 실시간 얼굴 감정 인식 (정확도 94%, D1 대비 7% 향상)
구성요소:
A. CNN 얼굴 검출부
B. 어텐션 기반 특징 강화 모듈 (D1의 FC 레이어를 변경)
C. 감정 분류부
...
출력:
{
  ...
  "non_obviousness": "D1의 FC 레이어를 어텐션 메커니즘으로 변경한 것은 단순 대체로 보이나, D1은 심층 CNN 구조에 최적화된 접근이라 어텐션 도입 동기가 부재했음. 이 변경 결과 정확도가 87%에서 94%로 7%p 향상되어 D1으로부터 예상 불가능한 효과가 확인되며, 통상의 파라미터 조정으로 도출 불가."
}

### 예시 2: 기술적 장벽 극복 + 시너지 (G06T)

[사용자 발명]
명칭: U-Net 개선 저조도 노이즈 제거 (PSNR 32.5dB, D1 대비 2.7dB 향상)
구성요소:
A. 개선된 U-Net 인코더
B. Attention Gate 모듈 (D1에는 없음)
C. 실시간 프레임 출력부
...
출력:
{
  ...
  "non_obviousness": "D1의 표준 U-Net에 Attention Gate 추가는 단순한 부품 추가로 보이나, 이 변경 시 계산 비용 증가로 실시간성 상실이라는 기술적 장벽이 있었음. 본 발명은 저조도 특화 학습으로 이를 극복하여 PSNR 2.7dB 향상과 실시간성을 동시 확보하는, D1의 U-Net(A)과의 결합 시너지로 도출된 예상 밖 효과 달성."
}
